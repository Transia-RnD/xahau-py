...
class TestRemit(TestCase):
    ...
    def test_uri_token_mint(self):
        transaction_dict = {
            ...
            "sequence": _SEQUENCE,
            "destination": _DESTINATION,
            "mint_uri_token": MintURIToken(uri=str_to_hex(_URI).upper(), digest=_DIGEST),
        }
        tx = Remit(**transaction_dict)
        self.assertTrue(tx.is_valid())
    
    # def test_bad_uri_token_mint(self):
    #     transaction_dict = {
    #         "account": _ACCOUNT,
    #         "fee": _FEE,
    #         "sequence": _SEQUENCE,
    #         "destination": _DESTINATION,
    #         "mint_uri_token": MintURIToken(uri=str_to_hex(_BAD_URI).upper(), digest=_DIGEST),
    #     }
    #     with self.assertRaises(XRPLModelException):
    #         Remit(**transaction_dict)
    ...
    def test_bad_digest(self):
        transaction_dict = {
            ...
            "sequence": _SEQUENCE,
            "destination": _DESTINATION,
            "mint_uri_token": MintURIToken(uri=str_to_hex(_URI).upper(), digest="DEADBEEF"),
        }
        with self.assertRaises(XRPLModelException):
            Remit(**transaction_dict)
