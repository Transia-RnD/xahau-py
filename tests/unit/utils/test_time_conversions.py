...
class TestTimeConversions(TestCase):
    def test_posix_round_trip(self):
        ...
        ripple_time = xahau.utils.posix_to_ripple_time(current_time)
        round_trip_time = xahau.utils.ripple_time_to_posix(ripple_time)
        self.assertEqual(time_whole_seconds, round_trip_time)

    def test_datetime_round_trip(self):
        now = datetime.now(timezone.utc)
        now_whole_seconds = now.replace(microsecond=0)
        ripple_time = xahau.utils.datetime_to_ripple_time(now)
        round_trip_time = xahau.utils.ripple_time_to_datetime(ripple_time)
        self.assertEqual(now_whole_seconds, round_trip_time)

    def test_ripple_epoch(self):
        dt = xahau.utils.ripple_time_to_datetime(0)
    ...
    def test_datetime_underflow(self):
        ...
        with self.assertRaises(xahau.utils.XAHLTimeRangeException):
            xahau.utils.datetime_to_ripple_time(year_1999)
    ...
    def test_posix_underflow(self):
        ...
        with self.assertRaises(xahau.utils.XAHLTimeRangeException):
            xahau.utils.posix_to_ripple_time(year_1999)
    ...
    def test_datetime_overflow(self):
        ...
        with self.assertRaises(xahau.utils.XAHLTimeRangeException):
            xahau.utils.datetime_to_ripple_time(year_2137)
    ...
    def test_posix_overflow(self):
        ...
        with self.assertRaises(xahau.utils.XAHLTimeRangeException):
            xahau.utils.posix_to_ripple_time(year_2137)
