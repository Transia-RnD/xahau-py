...
```bash
docker run -p 5005:5005 -p 6006:6006 --interactive -t --volume $PWD/.ci-config:/opt/xahau/etc/ --platform linux/amd64 xahauci/xahaud:latest /opt/xahau/bin/xahaud -a --conf /opt/xahau/etc/xahaud.cfg
```
...
* `xahaulabsofficial/xahaud:1.12.0` is an image that is regularly updated with the latest `xahaud` releases and can be found here: https://github.com/WietseWind/docker-xahaud
